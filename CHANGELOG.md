...
## [2026-01-27] - 10-Cycle Persistence & Audit (Protocol Jules x10)

### Added
- **Persistence**: Implemented PostgreSQL persistence for Clinical Records (`historias_clinicas` table).
- **Audit System**: Centralized audit logging via `auditMiddleware.ts` and `auditoria` table to track all mutations.
- **Billing Module**: New backend module for invoice management (`facturas` and `detalles_factura` tables).
- **RIPS Module**: Automated generation of US (Patients) and AC (Consultations) flat files for health reporting.

### Changed
- `backend/src/server.ts`: Registered new modules (Billing, RIPS) and integrated Audit Middleware.
- `ClinicalRecordService.ts`: Migrated from in-memory array to PostgreSQL pool.
- `backend/src/database/init.sql`: Expanded with 4 new tables and optimized indexes.

### Fixed
- **Integrity**: Guaranteed clinical record immutability for finalized records at the database level.
- **Traceability**: All sensitive API actions are now recorded with IP and User Agent.
