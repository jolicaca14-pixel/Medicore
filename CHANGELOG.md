# Changelog - MediCore Pro

All notable changes to this project will be documented in this file.

## [2026-01-23] - 10-Cycle Optimization (Protocol Jules)

### Added
- `hooks/useAuth.ts`: Custom hook for centralized authentication management.
- `utils/security.test.ts`: Unit tests for input sanitization.
- `utils/dataValidation.ts`: CIE-11 code validation utilities.
- `utils/finance.ts`: Colombian labor law surcharge calculations.
- `.jules/system_overview.md`: Comprehensive system documentation.

### Changed
- `App.tsx`:
    - Implemented secure session persistence using `sessionStorage`.
    - Integrated `useAuth` hook.
    - Improved accessibility with `aria-label`, `aria-live`, and `aria-required`.
- `components/views/ProfessionalView.tsx`:
    - Added clinical safety warning for missing antecedents during record finalization.
- `.Jules/Documentacion/task.md`: Updated to reflect Phase 4 completion.

### Fixed
- Fixed session loss on page refresh by implementing `sessionStorage` recovery.
- Improved clinical safety by preventing accidental finalization of records without history.

## [2026-01-23] - 20-Cycle Optimization (Protocol Jules x20)

### Added
- `components/ErrorBoundary.tsx`: Global error handler.
- `utils/auditLogger.ts`: Security auditing utility.
- `tests/auth.spec.ts`: Permanent Playwright E2E tests.
- `.jules/api_spec.md`: Data specification documentation.

### Changed
- `ProfessionalView.tsx`:
    - Dynamic progress messages during finalization.
    - Vital signs threshold alerts.
    - SOAT pricing integration in procedure selection.
- `utils/dataValidation.ts`: Added `validateCUPSCode`.
- `index.tsx`: Integrated `ErrorBoundary`.

## [2026-01-24] - 5-Cycle Advanced Optimization (Protocol Jules x5)

### Added
- `utils/clinicalLogic.ts`: Extracted clinical validation logic.

### Changed
- `ProfessionalView.tsx`:
    - Implemented reactive patient searching and filtering.
    - Added high-visibility Allergy alerts (Badges and Banner).
    - Integrated multi-point security audit logging.
- `types.ts`: Added `allergies` field to `Patient`.
- `constants.ts`: Updated mock data with clinical allergy records.

### Fixed
- **Security**: Removed hardcoded demo credentials from professional views.
- **UX**: Optimized patient list management for high-volume scenarios.

## [2026-01-24] - 20-Cycle Maturity Optimization (Protocol Jules x20)

### Added
- **Calculators**: BMI, CKD Stage, and Framingham color-coding utilities.
- **Persistence**: Auto-save drafts to `localStorage`.

### Changed
- **UX**:
    - "Clear Search" button and Empty States for patient list.
    - Sidebar tooltips and character counters for clinical fields.
    - "Copy ID" shortcut for administrative use.
- **Security**:
    - Input sanitization and audit logging for searches.
    - Sensitive data masking (PII) in system audit logs.
    - Digital signature presence validation.
- **Performance**: Debounced search functionality.

### Fixed
- **Safety**: Added pediatric-specific vital sign thresholds and gender-based clinical logic.

## [2026-03-17] - Cycle 12 Optimization Suite (Protocol Jules x10)

### Added
- `components/Skeleton.tsx`: Reusable loading state components.
- `components/SessionTimeoutHandler.tsx`: Security monitor for inactive sessions.
- `components/EmptyState.tsx`: High-impact UI for zero-result scenarios.

### Changed
- **UX**:
    - Integrated Skeleton Loaders in Patient and User directories.
    - Added Online/Offline status indicator in Layout header.
    - Implemented "Quick Note Templates" for faster clinical documentation.
    - Added rich Empty States with CTAs for search results.
- **Performance**:
    - Implemented Code Splitting (React.lazy/Suspense) for all main views, reducing initial bundle size.
- **Accessibility**:
    - Expanded Keyboard Shortcuts (Ctrl+F for search, Esc for close).
    - Added visual shortcut hints to the UI.
- **Security**:
    - Implemented 15-minute inactivity timeout with a 1-minute warning modal.
- **Finance**:
    - Added tax retention logic (ReteFuente/ICA) to `utils/finance.ts`.
    - Updated `BillingSummaryWidget` with net pay calculations for OPS contracts.
- **Data**:
    - Implemented LocalStorage draft recovery with Toast notifications.

### Fixed
- **Cleanup**: Automatic draft removal from `localStorage` upon record finalization.

## [2026-03-18] - Cycle 13 Optimization Suite (Protocol Jules x10)

### Added
- `utils/fetchWrapper.ts`: Robust fetch wrapper with backoff-retry logic (3 retries).
- `components/IntersectionVisible.tsx`: Intersection Observer component for lazy rendering.
- `hooks/useFocusTrap.ts`: Accessibility hook to manage keyboard focus in modals.

### Changed
- **UX**:
    - Enhanced `Toast.tsx` with support for optional actions (label/onClick).
    - Extended error toast visibility to 8 seconds.
    - Added interactive loading states (spinners and button disabling) to all primary action buttons.
- **Clinical**:
    - Implemented "Cargar Valores Normales" feature to instantly populate vitals with adult baselines.
- **Performance**:
    - Integrated lazy rendering in the patient list to improve DOM performance.
- **Security**:
    - Hardened `index.html` with a strict Content Security Policy (CSP) meta tag.
- **Finance**:
    - Added "Professional Earnings Projection" in Reports, calculating gross production and net income after retentions.
- **Data**:
    - Implemented Clinical Record PDF simulation (Print view).
    - Replaced contract download placeholders with real Blob-based file downloads.

### Fixed
- **Accessibility**: Applied focus trapping to RDA and Auth modals to improve screen reader and keyboard navigation.
- **Stability**: Resolved duplicated function declarations and redundant imports in `ProfessionalView.tsx`.

---
*Generated by Jules (Lead Engineer) during Cycle 71 (Phase 13).*
