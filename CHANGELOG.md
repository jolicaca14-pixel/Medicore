...
## [2026-01-24] - Persona Expansion & Accessibility (Protocol Jules/Palette)

### Added
- **Protocols**: Created 10 specialized agent protocols in `.Jules/` (Psychologist, Accountant, Nutritionist, Secretary, Treasurer, HR Manager, Contract Assistant, Manager, Bacteriologist, and Radiologist).
- **Audit Suite**: Implemented `tests/persona_audit.spec.ts` for automated E2E workflow simulation across all new roles.

### Changed
- **RBAC & Rendering**: Refactored `App.tsx` and `Layout.tsx` to handle multi-role users gracefully by resolving ID collisions and implementing mutually exclusive view domains.
- **Domain Separation**: Unique tab IDs for clinical ('my_production') vs institutional ('financial_mgmt') modules.

### Fixed
- **Accessibility**: Added missing `htmlFor` and `id` linking in dynamic form fields (ProfessionalView) and appointment modals (SecretaryView).
- **UI Conflict**: Eliminated dashboard/report overlaps for users holding both clinical and administrative roles.
