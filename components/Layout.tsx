...
export const Layout: React.FC<LayoutProps> = ({ user, onLogout, children, activeTab, setActiveTab }) => {
  ...
  const getMenuItems = () => {
    ...
    if (roles.includes(UserRole.SECRETARY)) {
       ...
       items.push({ id: 'billing', label: 'Facturación', icon: DollarSign });
       items.push({ id: 'cartera', label: 'Cartera', icon: Briefcase });
    }
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shadow-sm z-10">
        ...
        <div className="p-4 border-t border-slate-100">
          <button
            ...
            onClick={() => isSystemAdmin(user.roles) && setActiveTab('settings')}
