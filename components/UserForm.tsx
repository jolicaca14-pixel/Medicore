  ...
  const handleSave = () => {
    ...
    if (
      ...
      if (!currentUser.professionalLicense) {
        setError('Para roles asistenciales, el Registro Médico/Profesional es obligatorio.');
        return;
      }
    }
    onSave(currentUser);
  ...
  const FormContent = () => (
    <>
      <div className="grid grid-cols-2 gap-4">
        ...
        {error && (
            <div className="mt-4 p-3 bg-red-50 border border-red-200 text-red-600 text-xs font-bold rounded-lg animate-in fade-in zoom-in duration-200">
                {error}
            </div>
        )}
