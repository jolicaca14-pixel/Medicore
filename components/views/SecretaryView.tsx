                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {patients
                                .filter(p => p.fullName.toLowerCase().includes(billingSearchTerm.toLowerCase()) || p.identification.includes(billingSearchTerm))
                                .map(p => (
                                <tr key={p.id} className="hover:bg-slate-50">
                                    <td className="p-4 font-bold text-slate-700">{p.fullName}</td>
                                    <td className="p-4">{p.identification}</td>
                                    <td className="p-4 text-xs font-bold text-blue-600 uppercase">{p.insuranceType}</td>
                                    <td className="p-4 text-slate-500">{p.phone}</td>
                                    <td className="p-4 text-right">
                                        <button onClick={() => { setBillingPatient(p); setActiveTab('BILLING'); }} className="text-blue-600 font-bold hover:underline" title="Facturar">Facturar</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                       ...
                       <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                           <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6">
                               ...
                               <div className="space-y-4">
                                   ...
                                   <div>
                                       <label className="block text-sm font-bold text-slate-700 mb-1">Monto a Recibir</label>
                                       <input
                                           type="number"
                                           className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 text-lg font-bold"
                                           value={paymentAmount}
                               ...
                               <div className="flex justify-end gap-2 mt-6">
                                   ...
                                   <button onClick={confirmPayment} className="px-4 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700">Confirmar Pago</button>
                               </div>
                           </div>
                       </div>
                   )}
