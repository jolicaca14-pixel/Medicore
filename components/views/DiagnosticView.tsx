...
export const DiagnosticView: React.FC<DiagnosticViewProps> = ({ user, onLogout }) => {
  ...
  const handlePrintDate = (patientId: string, date: string) => {
      const recordsToPrint = completedRecords.filter(r => r.patientId === patientId && r.dateCreated.startsWith(date));
      ...
      if (printWindow) {
          printWindow.document.write(`
            <html>
                <head>
                    <title>Reporte de Resultados - ${patient?.fullName}</title>
                    <style>
                        ...
                        .result-title { color: #0284c7; font-weight: bold; font-size: 1.2em; margin-bottom: 10px; border-bottom: 1px solid #e2e8f0; padding-bottom: 5px; }
                ...
                <body>
                    <div class="header">
                        <h1>MediCore Pro - Reporte de Resultados</h1>
                        <p>Fecha del Reporte: ${new Date().toLocaleDateString()} | Origen: ${user.roles.includes(UserRole.BACTERIOLOGIST) ? 'Laboratorio Clínico' : 'Imagenología'}</p>
                    </div>

                    <div class="patient-info">
                        <div class="data-grid">
                            <div><span class="label">Paciente:</span> ${patient?.fullName}</div>
                            <div><span class="label">Identificación:</span> ${patient?.identification}</div>
                            <div><span class="label">Fecha Toma:</span> ${date}</div>
                        </div>
                    </div>
                    ...
                    <footer>
                        <p>Este documento es una representación electrónica de resultados clínicos. Firmado digitalmente por ${user.name} (${user.professionalLicense}).</p>
                    </footer>
                </body>
            </html>
          `);
