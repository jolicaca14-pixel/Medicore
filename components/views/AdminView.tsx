...
export const AdminView: React.FC<AdminViewProps> = ({ activeTab, setActiveTab, currentUserSession }) => {
  ...
  const handleRejectPayment = (reqId: string) => {
      if (!window.confirm("¿Está seguro de RECHAZAR esta cuenta de cobro?")) return;
      setPaymentRequests(prev => prev.map(req => req.id === reqId ? { ...req, status: 'REJECTED' } : req));
      showStatus("Cuenta de cobro rechazada.");
  };
  ...
  const generateRIPS = () => {
      ...
      if (filteredRecords.length === 0) {
          showStatus("No se encontraron registros finalizados.", 'ERROR');
  ...
  const handleNewTemplate = () => {
      setCurrentTemplate({ name: '', description: '', allowedRoles: [UserRole.PROFESSIONAL], active: true, sections: [], recordType: RecordType.GENERAL });
      setIsTemplateModalOpen(true);
  };
  const handleNewSection = () => {
      setCurrentSection({ title: '', fields: [] });
      setIsSectionModalOpen(true);
  };
  const handleNewField = () => {
      setCurrentField({ id: '', label: '', type: 'TEXT', required: false });
      setIsFieldModalOpen(true);
  };

  const handleSaveTemplate = () => {
      if (!currentTemplate.name) return showStatus("Nombre obligatorio", 'ERROR');
  ...
  const handleSaveSection = () => {
      if (!currentSection.title) return showStatus("Título obligatorio", 'ERROR');
  ...
  const handleSaveField = () => {
      if (!currentField.id || !currentField.label) return showStatus("ID y Etiqueta obligatorios", 'ERROR');
  ...
  if ((activeTab === 'dashboard' || activeTab === 'hr' || activeTab === 'reports') && !hasAdminAccess) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-slate-400">
            <Ban size={64} className="mb-4 text-red-400"/>
            <h2 className="text-xl font-bold text-slate-700">Acceso Denegado</h2>
            <p className="text-sm">No tiene los privilegios administrativos necesarios.</p>
        </div>
    );
  }
  ...
  if (activeTab === 'users' && isAdmin) {
      return (
        <div className="space-y-6 animate-in fade-in duration-500">
            {/* User Form Space (Top) */}
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 ring-4 ring-slate-50">
                 <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-6">
                    <div>
                        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Centro de Gestión de Identidades</h2>
                        <h3 className="text-lg font-bold text-blue-600 mt-1 flex items-center">
                            <Plus size={20} className="mr-2"/> Registrar Nuevo Colaborador
                        </h3>
                        <p className="text-sm text-slate-500 mt-2">Configure los accesos, roles y credenciales profesionales del personal de la IPS.</p>
                    </div>
                    <button onClick={handleAddNewUser} className="px-6 py-3 bg-slate-900 text-white text-sm font-bold rounded-xl flex items-center hover:bg-slate-800 transition-all shadow-lg active:scale-95">
                        <Plus size={18} className="mr-2"/> Nuevo Colaborador
                    </button>
                 </div>
                 <div className="mt-8 bg-slate-50/50 p-6 rounded-2xl border border-dashed border-slate-200">
                    <UserForm
                        user={currentUser}
                        onSave={(u) => { handleSaveUser(u); showStatus("Usuario actualizado correctamente"); }}
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
              ...
              {isTemplateModalOpen && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                        <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center">
                            <LayoutTemplate className="mr-2 text-blue-600"/> {currentTemplate.id ? 'Editar Plantilla' : 'Nueva Plantilla'}
                        </h3>
                        <div className="space-y-4">
                            <div>
                            ...
                            <div>
                                ...
                                <textarea className="w-full border p-2 rounded" rows={3} value={currentTemplate.description || ''} onChange={e => setCurrentTemplate({...currentTemplate, description: e.target.value})} />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Tipo de Registro</label>
                                <select className="w-full border p-2 rounded" value={currentTemplate.recordType} onChange={e => setCurrentTemplate({...currentTemplate, recordType: e.target.value as any})}>
                        ...
                        <div className="flex justify-end gap-2 mt-6">
                            ...
                            <button onClick={handleSaveTemplate} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold">Guardar</button>
                        </div>
                    </div>
                </div>
              )}
              {isSectionModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                        <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center">
                            <Layers className="mr-2 text-blue-600"/> {currentSection.id ? 'Editar Sección' : 'Nueva Sección'}
                        </h3>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Título de Sección</label>
                            ...
                            <div>
                                ...
                                <textarea className="w-full border p-2 rounded" rows={2} value={currentSection.description || ''} onChange={e => setCurrentSection({...currentSection, description: e.target.value})} />
                            </div>
                        </div>
                        <div className="flex justify-end gap-2 mt-6">
                            <button onClick={() => setIsSectionModalOpen(false)} className="px-4 py-2 text-slate-600 font-bold">Cancelar</button>
                            <button onClick={handleSaveSection} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold">Guardar</button>
                        </div>
                      </div>
                  </div>
              )}
              {isFieldModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                        <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center">
                            <Type className="mr-2 text-blue-600"/> {currentField.id ? 'Editar Campo' : 'Nuevo Campo'}
                        </h3>
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">ID Único (Variable)</label>
                                    <input className="w-full border p-2 rounded font-mono text-xs" value={currentField.id || ''} onChange={e => setCurrentField({...currentField, id: e.target.value})} placeholder="ej: global_bp" />
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <input className="w-full border p-2 rounded" value={currentField.label || ''} onChange={e => setCurrentField({...currentField, label: e.target.value})} placeholder="ej: Presión Arterial" />
                            </div>
                        ...
                        <div className="flex justify-end gap-2 mt-6">
                            ...
                            <button onClick={handleSaveField} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold">Guardar</button>
                        </div>
                      </div>
                  </div>
              )}
                              ...
                              <div key={t.id} className="border border-slate-200 rounded-xl p-5 hover:shadow-md transition-shadow relative group bg-slate-50/50">
                                  <div className="flex justify-between items-start mb-2">
                                      ...
                                      <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                          ...
                                          <button onClick={() => { setTemplates(prev => prev.filter(item => item.id !== t.id)); showStatus("Plantilla eliminada"); }} className="p-1.5 bg-white border rounded hover:text-red-600"><Trash2 size={14}/></button>
                                      </div>
                                  </div>
