...
export const AdminView: React.FC<AdminViewProps> = ({ activeTab, setActiveTab, currentUserSession }) => {
  ...
  const handleNewTemplate = () => {
    setCurrentTemplate({ id: `t_${Date.now()}`, name: '', description: '', active: true, allowedRoles: [], sections: [], recordType: RecordType.GENERAL });
    setIsTemplateModalOpen(true);
  };
  const handleNewSection = () => {
    setCurrentSection({ id: `sec_${Date.now()}`, title: '', description: '', fields: [] });
    setIsSectionModalOpen(true);
  };
  const handleNewField = () => {
    setCurrentField({ id: `f_${Date.now()}`, label: '', type: 'TEXT' as FieldType, required: false, isGlobal: true });
    setIsFieldModalOpen(true);
  };

  const handleSaveTemplate = () => {
    if (!currentTemplate.name || !currentTemplate.recordType) return alert("Nombre y Tipo de Registro son obligatorios");
    if (templates.some(t => t.id === currentTemplate.id)) {
        ...
        setTemplates(prev => [...prev, currentTemplate as RoleTemplate]);
    }
  ...
  const handleSaveSection = () => {
    ...
    if (globalSections.some(s => s.id === currentSection.id)) {
        ...
        setGlobalSections(prev => [...prev, currentSection as TemplateSection]);
    }
    setIsSectionModalOpen(false);
  };

  const handleSaveField = () => {
    if (!currentField.label || !currentField.type) return alert("Etiqueta y Tipo son obligatorios");
  ...
  const restrictedTabsForNonAdmins = ['users', 'settings', 'files'];
  if (restrictedTabsForNonAdmins.includes(activeTab) && !isAdmin) {
      return (
  ...
  if ((activeTab === 'hr' || activeTab === 'reports' || activeTab === 'dashboard') && !hasAdministrativeAccess) {
    return (
        <div className="flex flex-col items-center justify-center h-full text-slate-400">
            <Ban size={64} className="mb-4 text-red-400"/>
            <h2 className="text-xl font-bold text-slate-700">Acceso Restringido</h2>
            <p className="text-sm">Se requieren permisos Administrativos (Admin, Gerente o Contador) para acceder a este módulo.</p>
        </div>
    );
  }
  ...
  const renderTabContent = () => {
    // 1. DASHBOARD (Dynamic & Actionable) - For Admins, Managers, Accountants
                            ...
                            <div className="col-span-2">
                                ...
                                <div className="flex flex-wrap gap-2 mt-1">
                                    {Object.values(UserRole).map(role => (
                                        <label key={role} className="flex items-center space-x-1 bg-slate-50 p-1.5 rounded border text-[10px]">
                                            <input
                                                type="checkbox"
                      ...
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          ...
                          <div className="space-y-4">
                              ...
                              <div>
                                  ...
                                  <textarea className="w-full border p-2 rounded" value={currentSection.description || ''} onChange={e => setCurrentSection({...currentSection, description: e.target.value})} />
                              </div>
                              <div>
                                  <label className="text-xs font-bold text-slate-500">Campos en esta Sección</label>
                                  <div className="bg-slate-50 p-2 rounded border max-h-40 overflow-y-auto">
                                      {globalFields.map(f => (
                                          <label key={f.id} className="flex items-center space-x-2 p-1 hover:bg-white rounded">
                                              <input
                                                type="checkbox"
                                                checked={currentSection.fields?.some(sf => sf.id === f.id)}
                                                onChange={() => {
                                                    const fields = currentSection.fields || [];
                                                    const newFields = fields.some(sf => sf.id === f.id)
                                                        ? fields.filter(sf => sf.id !== f.id)
                                                        : [...fields, f];
                      ...
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              ...
                              <div>
                                  ...
                                  <input className="w-full border p-2 rounded" placeholder="Ej: kg, cm, mg/dL" value={currentField.unit || ''} onChange={e => setCurrentField({...currentField, unit: e.target.value})} />
                              </div>
                              <div className="col-span-2">
                              ...
                              {currentField.type === 'CALCULATED' && (
                                  <div className="col-span-2">
                                      <label className="text-xs font-bold text-slate-500">Fórmula / Script</label>
                                      <textarea className="w-full border p-2 rounded font-mono text-xs" placeholder="Ej: (w / (h*h))" value={currentField.formula || ''} onChange={e => setCurrentField({...currentField, formula: e.target.value})} />
                                  </div>
                              )}
                          </div>
                          <div className="flex justify-end gap-2 mt-6">
                              ...
                              <div key={t.id} className="border border-slate-200 rounded-xl p-5 hover:shadow-md transition-shadow relative group bg-slate-50/50">
                                  <div className="flex justify-between items-start mb-2">
                                      ...
                                      <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                          ...
                                          <button onClick={() => window.confirm('¿Eliminar esta plantilla?') && setTemplates(prev => prev.filter(tmp => tmp.id !== t.id))} className="p-1.5 bg-white border rounded hover:text-red-600"><Trash2 size={14}/></button>
                                      </div>
                                  </div>
    ...
    return (
        <div className="flex flex-col items-center justify-center h-64 text-slate-400">
            ...
            <p className="text-sm italic">Módulo en desarrollo o no disponible para su rol.</p>
        </div>
    );
  };
