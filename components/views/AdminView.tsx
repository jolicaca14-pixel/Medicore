...
export const AdminView: React.FC<AdminViewProps> = ({ activeTab, setActiveTab, currentUserSession }) => {
  ...
  const isManagement = currentUserSession?.roles.includes(UserRole.MANAGER) || currentUserSession?.roles.includes(UserRole.ACCOUNTANT);

  if ((activeTab === 'users' || activeTab === 'settings' || activeTab === 'files') && !isSystemAdmin) {
      return (
  ...
  const renderTabContent = () => {
    // 1. DASHBOARD (Dynamic & Actionable) - Only for Admins/Managers
    if (activeTab === 'dashboard' && (isSystemAdmin || isManagement)) {
    ...
    if (activeTab === 'hr' && (isSystemAdmin || isManagement)) {
        return (
            <div className="space-y-6">
              {/* MODALS */}
    ...
    if (activeTab === 'reports' && (isSystemAdmin || isManagement)) {
        return (
                 ...
                 <div className="relative z-10">
                    <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-4">
                        <div>
                            <h2 className="text-2xl font-bold flex items-center">
                                <Shield className="mr-3 text-blue-400"/> Centro de Gestión de Identidades
                            </h2>
                            <p className="text-slate-400 text-sm mt-1">Registrar, editar y auditar accesos de colaboradores de MediCore Pro.</p>
                        </div>
                        <button onClick={handleAddNewUser} className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold rounded-xl flex items-center transition-all shadow-lg transform hover:-translate-y-0.5">
                    ...
                    <div className="bg-white/5 backdrop-blur-md rounded-xl p-6 border border-white/10">
                        <h3 className="text-lg font-bold mb-4 text-blue-300 flex items-center">
                            {currentUser.id && users.some(u => u.id === currentUser.id) ? <Edit size={18} className="mr-2"/> : <Plus size={18} className="mr-2"/>}
                            {currentUser.id && users.some(u => u.id === currentUser.id) ? `Editando Perfil: ${currentUser.name}` : 'Formulario de Registro Rápido'}
                        </h3>
                        <div className="bg-white rounded-xl p-6 text-slate-800">
                            <UserForm
                                user={currentUser}
                                onSave={(u) => { handleSaveUser(u); showStatus("Usuario guardado correctamente"); }}
        ...
        return (
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              {isTemplateModalOpen && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                        <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center"><LayoutTemplate className="mr-2 text-blue-600"/> {templates.some(t => t.id === currentTemplate.id) ? 'Editar Plantilla' : 'Nueva Plantilla'}</h3>
                        <div className="space-y-4">
                            <div>
                                <label htmlFor="tpl-name" className="block text-xs font-bold text-slate-500 mb-1">Nombre de la Plantilla</label>
                                <input id="tpl-name" className="w-full border p-2 rounded text-sm" value={currentTemplate.name || ''} onChange={e => setCurrentTemplate({...currentTemplate, name: e.target.value})} placeholder="Ej: Consulta General" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1">Descripción</label>
                                <textarea className="w-full border p-2 rounded text-sm" value={currentTemplate.description || ''} onChange={e => setCurrentTemplate({...currentTemplate, description: e.target.value})} rows={2} />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                ...
                                <div className="flex items-center mt-6">
                                    <input type="checkbox" checked={currentTemplate.active} onChange={e => setCurrentTemplate({...currentTemplate, active: e.target.checked})} className="mr-2" id="tpl-active" />
                                    <label htmlFor="tpl-active" className="text-xs font-bold text-slate-700">Plantilla Activa</label>
                                </div>
                            </div>
                        </div>
                        <div className="flex justify-end gap-2 mt-6">
              ...
              {isSectionModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center"><Layers className="mr-2 text-blue-600"/> {globalSections.some(s => s.id === currentSection.id) ? 'Editar Sección' : 'Nueva Sección'}</h3>
                          <div className="space-y-4">
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 mb-1">Título de la Sección</label>
                                  <input className="w-full border p-2 rounded text-sm" value={currentSection.title || ''} onChange={e => setCurrentSection({...currentSection, title: e.target.value})} placeholder="Ej: Antecedentes" />
                              </div>
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 mb-1">Descripción</label>
                                  <textarea className="w-full border p-2 rounded text-sm" value={currentSection.description || ''} onChange={e => setCurrentSection({...currentSection, description: e.target.value})} rows={2} />
                              </div>
                          </div>
                          <div className="flex justify-end gap-2 mt-6">
              ...
              {isFieldModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center"><Type className="mr-2 text-blue-600"/> {globalFields.some(f => f.id === currentField.id) ? 'Editar Campo' : 'Nuevo Campo'}</h3>
                          <div className="space-y-4">
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 mb-1">Etiqueta (Label)</label>
                                  <input className="w-full border p-2 rounded text-sm" value={currentField.label || ''} onChange={e => setCurrentField({...currentField, label: e.target.value})} placeholder="Ej: Tensión Arterial" />
                              </div>
                              <div className="grid grid-cols-2 gap-4">
                                  <div>
                                  ...
                                  <div>
                                      ...
                                      <input className="w-full border p-2 rounded text-sm" value={currentField.unit || ''} onChange={e => setCurrentField({...currentField, unit: e.target.value})} placeholder="Ej: mmHg" />
                                  </div>
                              </div>
                              <div className="flex items-center">
                                  <input type="checkbox" checked={currentField.required} onChange={e => setCurrentField({...currentField, required: e.target.checked})} className="mr-2" id="field-required" />
                                  <label htmlFor="field-required" className="text-xs font-bold text-slate-700">Obligatorio</label>
                              </div>
                          </div>
                          <div className="flex justify-end gap-2 mt-6">
