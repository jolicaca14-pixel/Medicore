...
export const AdminView: React.FC<AdminViewProps> = ({ activeTab, setActiveTab, currentUserSession }) => {
  ...
  const handleDeleteFile = (fileId: string, type: 'CONTRACT' | 'PAYMENT' | 'CLINICAL') => {
      ...
      if (type === 'CONTRACT') {
          ...
          // CLINICAL - Since records are mock constants, we simulate removal in state if it were fully dynamic
          alert("Archivo clínico marcado para eliminación en el servidor.");
      }
  ...
  const userRoles = currentUserSession?.roles || [];

  // Specific restrictions for ADMIN only tabs
  const isAdminOnlyTab = ['users', 'settings', 'files'].includes(activeTab);
  const isHRorReportsTab = ['hr', 'reports', 'dashboard'].includes(activeTab);

  if (isAdminOnlyTab && !isSystemAdmin(userRoles)) {
      return (
  ...
  if (isHRorReportsTab && !hasAdministrativeAccess(userRoles)) {
      return (
          <div className="flex flex-col items-center justify-center h-full text-slate-400">
              <Ban size={64} className="mb-4 text-red-400"/>
              <h2 className="text-xl font-bold text-slate-700">Acceso Restringido</h2>
  ...
  if (activeTab === 'dashboard' && hasAdministrativeAccess(userRoles)) {
                ...
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h3 className="font-bold text-lg text-slate-800">
                            {fileManagementTab === 'CONTRACTS' ? 'Archivos de Contratos' :
                             fileManagementTab === 'PAYMENTS' ? 'Archivos de Soportes de Pago' :
                             'Anexos y Resultados Clínicos'}
                        </h3>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {(fileManagementTab === 'CONTRACTS'
                            ...
                            : MOCK_RECORDS.filter(r => r.attachments && r.attachments.length > 0).flatMap(r => r.attachments.map(a => ({ ...a, userName: MOCK_PATIENTS.find(p => p.id === r.patientId)?.fullName, fileUrl: a.name, type: 'CLINICAL' })))
                        )
                            .filter(file =>
                                (file.fileUrl?.toLowerCase().includes(fileSearchTerm.toLowerCase()) ||
                                (file.userName && file.userName.toLowerCase().includes(fileSearchTerm.toLowerCase())))
                            )
                            .map((file: any) => (
                                <tr key={file.id}>
                                    <td className="p-3 font-medium text-slate-700">{file.fileUrl || `archivo_${file.id}.pdf`}</td>
                                    <td className="p-3">{file.userName}</td>
                                    <td className="p-3 text-slate-500">{new Date(file.startDate || file.dateSubmitted || file.date).toLocaleDateString()}</td>
                        ...
                        {fileManagementTab === 'CLINICAL' && MOCK_RECORDS.every(r => !r.attachments || r.attachments.length === 0) && (
                            <tr><td colSpan={4} className="p-8 text-center text-slate-400 italic">No hay anexos clínicos registrados.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
  ...
  if (activeTab === 'hr' && hasAdministrativeAccess(userRoles)) {
      return (
          <div className="space-y-6">
              {/* MODALS */}
  ...
  if (activeTab === 'reports' && hasAdministrativeAccess(userRoles)) {
      return (
                    ...
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-slate-800">
                                {currentUser.id && users.some(u => u.id === currentUser.id) ? 'Editando Perfil de Usuario' : 'Centro de Registro de Usuarios'}
                            </h3>
                            <p className="text-sm text-slate-500">Gestione identidades y privilegios de acceso al sistema.</p>
                        </div>
                    </div>
      ...
      return (
          <div className="space-y-6 animate-in fade-in duration-500">
              ...
              {isTemplateModalOpen && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                        <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center"><LayoutTemplate className="mr-2 text-blue-600"/> Configuración de Plantilla</h3>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1">Nombre de la Plantilla</label>
                                <input className="w-full p-2 border rounded text-sm" placeholder="Ej: Consulta General" />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1">Descripción</label>
                                <textarea className="w-full p-2 border rounded text-sm" rows={3} placeholder="Breve descripción del propósito..." />
                            </div>
                            <div>
                        ...
                        <div className="mt-6 flex justify-end gap-2">
                            ...
                            <button onClick={() => { setIsTemplateModalOpen(false); alert("Plantilla guardada (Simulación)"); }} className="px-4 py-2 bg-blue-600 text-white rounded font-bold text-sm">Guardar Cambios</button>
                        </div>
                    </div>
                </div>
              )}
              {isSectionModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center"><Layers className="mr-2 text-indigo-600"/> Nueva Sección Clínica</h3>
                          <div className="space-y-4">
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 mb-1">Título de la Sección</label>
                                  <input className="w-full p-2 border rounded text-sm" placeholder="Ej: Examen Físico" />
                              </div>
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 mb-1">ID Único (Opcional)</label>
                                  <input className="w-full p-2 border rounded text-sm font-mono" placeholder="sec_examen_fisico" />
                              </div>
                          </div>
                          <div className="mt-6 flex justify-end gap-2">
                              <button onClick={() => setIsSectionModalOpen(false)} className="px-4 py-2 text-slate-600 font-bold text-sm">Cancelar</button>
                              <button onClick={() => { setIsSectionModalOpen(false); alert("Sección agregada a la biblioteca"); }} className="px-4 py-2 bg-indigo-600 text-white rounded font-bold text-sm">Crear Sección</button>
                          </div>
                      </div>
                  </div>
              )}
              {isFieldModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center"><Type className="mr-2 text-purple-600"/> Definición de Campo / Variable</h3>
                          <div className="space-y-4">
                              <div className="grid grid-cols-2 gap-4">
                                  <div className="col-span-2">
                                      <label className="block text-xs font-bold text-slate-500 mb-1">Etiqueta (Label)</label>
                                      <input className="w-full p-2 border rounded text-sm" placeholder="Ej: Tensión Arterial Sistólica" />
                                  </div>
                                  <div>
                                  ...
                                  <div>
                                      ...
                                      <input className="w-full p-2 border rounded text-sm" placeholder="Ej: mmHg" />
                                  </div>
                              </div>
                          </div>
                          <div className="mt-6 flex justify-end gap-2">
                              <button onClick={() => setIsFieldModalOpen(false)} className="px-4 py-2 text-slate-600 font-bold text-sm">Cancelar</button>
                              <button onClick={() => { setIsFieldModalOpen(false); alert("Campo registrado exitosamente"); }} className="px-4 py-2 bg-purple-600 text-white rounded font-bold text-sm">Registrar Campo</button>
                          </div>
                      </div>
                  </div>
              )}
