...
export const AdminView: React.FC<AdminViewProps> = ({ activeTab, setActiveTab, currentUserSession }) => {
  ...
  const restrictedTabs = ['hr', 'reports', 'dashboard', ...adminOnlyTabs];

  if (restrictedTabs.includes(activeTab) && !hasAdministrativeAccess) {
      return (
  ...
  if (adminOnlyTabs.includes(activeTab) && !isAdmin) {
    return (
        <div className="flex flex-col items-center justify-center h-full text-slate-400">
            ...
            <h2 className="text-xl font-bold text-slate-700">Módulo Reservado</h2>
            <p className="text-sm">Este módulo es de uso exclusivo del ADMINISTRADOR del sistema.</p>
        </div>
    );
  }
  ...
  if (activeTab === 'users' && isAdmin) {
      return (
        <div className="space-y-6 animate-in fade-in duration-500">
            {/* Centro de Gestión de Identidades - User Form Space (Top) */}
            <div className="bg-slate-50 p-8 rounded-2xl shadow-inner border border-slate-200">
                 <div className="flex justify-between items-center mb-6 border-b border-slate-200 pb-4">
                    <div>
                        <h3 className="text-xl font-bold text-slate-900 flex items-center">
                            <Shield className="mr-2 text-blue-600"/>
                            {currentUser.id && users.some(u => u.id === currentUser.id) ? 'Centro de Edición de Perfil' : 'Centro de Creación de Identidades'}
                        </h3>
                        <p className="text-sm text-slate-600">Gestión centralizada de credenciales y roles para el personal de la institución.</p>
                    </div>
      ...
      return (
          <div className="space-y-6 animate-in fade-in duration-500">
              ...
              {isTemplateModalOpen && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                        <h3 className="text-lg font-bold text-slate-800 mb-4">Nueva Plantilla Clínica</h3>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1">Nombre de la Plantilla</label>
                                <input type="text" className="w-full border p-2 rounded text-sm" placeholder="Ej: Evolución Médica General" />
                            </div>
                            <div>
                                ...
                                <select className="w-full border p-2 rounded text-sm">
                                    ...
                                    <option value="PROCEDURE">Nota de Procedimiento</option>
                                    <option value="PYP_GROWTH_DEV">PyP Crecimiento y Des.</option>
                                </select>
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1">Descripción</label>
                                <textarea className="w-full border p-2 rounded text-sm" rows={2} placeholder="Describa el propósito de esta plantilla..."></textarea>
                            </div>
                        </div>
                        <div className="flex justify-end gap-2 mt-6">
                            <button onClick={() => setIsTemplateModalOpen(false)} className="px-4 py-2 text-slate-600 font-bold text-sm">Cancelar</button>
                            <button onClick={() => { setIsTemplateModalOpen(false); alert('Plantilla guardada (Simulación)'); }} className="px-4 py-2 bg-slate-900 text-white rounded font-bold text-sm">Guardar Plantilla</button>
                        </div>
                    </div>
                </div>
              )}
              {isSectionModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          <h3 className="text-lg font-bold text-slate-800 mb-4">Nueva Sección de Historia</h3>
                          <div className="space-y-4">
                            <div>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1">ID Único (Cuerpo RDA)</label>
                                <input type="text" className="w-full border p-2 rounded font-mono text-xs" placeholder="s_antecedentes_fam" />
                            </div>
                            <div className="bg-blue-50 p-3 rounded border border-blue-100">
                                <p className="text-[10px] text-blue-800 font-bold mb-1 uppercase">Información:</p>
                                <p className="text-[10px] text-blue-700">Podrá agregar campos a esta sección una vez guardada desde la biblioteca de campos.</p>
                            </div>
                          </div>
                          <div className="flex justify-end gap-2 mt-6">
                            <button onClick={() => setIsSectionModalOpen(false)} className="px-4 py-2 text-slate-600 font-bold text-sm">Cancelar</button>
                            <button onClick={() => { setIsSectionModalOpen(false); alert('Sección creada (Simulación)'); }} className="px-4 py-2 bg-slate-900 text-white rounded font-bold text-sm">Crear Sección</button>
                          </div>
                      </div>
                  </div>
              )}
                  ...
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          ...
                          <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="text" className="w-full border p-2 rounded text-sm" placeholder="Ej: Peso Actual" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 mb-1">ID Variable</label>
                                    <input type="text" className="w-full border p-2 rounded text-sm font-mono" placeholder="global_weight" />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                ...
                                <div>
                                    ...
                                    <input type="text" className="w-full border p-2 rounded text-sm" placeholder="Ej: kg, mmHg, cm" />
                                </div>
                            </div>
                            <div>
                          ...
                          <div className="flex justify-end gap-2 mt-6">
                            ...
                            <button onClick={() => { setIsFieldModalOpen(false); alert('Campo global guardado (Simulación)'); }} className="px-4 py-2 bg-slate-900 text-white rounded font-bold text-sm">Guardar Campo</button>
                          </div>
                      </div>
                  </div>
              )}
