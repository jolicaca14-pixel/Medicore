...
export const AdminView: React.FC<AdminViewProps> = ({ activeTab, setActiveTab, currentUserSession }) => {
  ...
  const handleNewTemplate = () => {
      setCurrentTemplate({ name: '', description: '', allowedRoles: [UserRole.PROFESSIONAL], sections: [], recordType: RecordType.GENERAL, active: true });
  ...
  const handleSaveSection = () => {
      ...
      const sectionToSave = {
          ...currentSection,
          id: currentSection.id || `sec-${Date.now()}`,
  ...
  const handleNewSection = () => {
      setCurrentSection({ title: '', description: '', fields: [] });
  ...
  const handleSaveField = () => {
      ...
      const fieldToSave = {
          ...currentField,
          id: currentField.id || `fld-${Date.now()}`,
  ...
  const handleNewField = () => {
      setCurrentField({ label: '', type: 'TEXT', required: false, unit: '', placeholder: '' });
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
              ...
              {isTemplateModalOpen && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-bold text-slate-800">{currentTemplate.id ? 'Editar Plantilla' : 'Nueva Plantilla'}</h3>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Object.values(UserRole).map(role => (
                                        <label key={role} className="flex items-center space-x-1 text-xs">
                                            <input
                                                type="checkbox"
                                                checked={currentTemplate.allowedRoles?.includes(role)}
                                                onChange={() => {
                                                    const current = currentTemplate.allowedRoles || [];
                                                    const updated = current.includes(role) ? current.filter(r => r !== role) : [...current, role];
                                                    setCurrentTemplate({...currentTemplate, allowedRoles: updated});
                                                }}
                                            />
                        ...
                        <div className="mt-6 flex justify-end gap-2">
                            ...
                            <button onClick={handleSaveTemplate} className="px-4 py-2 bg-slate-900 text-white rounded font-bold text-sm">Guardar Plantilla</button>
                        </div>
                    </div>
                </div>
              )}
              {isSectionModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full p-6">
                          <div className="flex justify-between items-center mb-4">
                              <h3 className="text-lg font-bold text-slate-800">{currentSection.id ? 'Editar Sección' : 'Nueva Sección'}</h3>
                          ...
                          <div className="space-y-4">
                              ...
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 mb-1">Campos de la Sección</label>
                                  <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto border p-2 rounded bg-slate-50">
                                      {globalFields.map(field => (
                                          <label key={field.id} className="flex items-center space-x-2 text-xs p-1 hover:bg-white rounded border border-transparent hover:border-slate-200">
                                              <input
                                                  type="checkbox"
                                                  checked={currentSection.fields?.some(f => f.id === field.id)}
                                                  onChange={() => {
                                                      const current = currentSection.fields || [];
                                                      const updated = current.some(f => f.id === field.id)
                                                          ? current.filter(f => f.id !== field.id)
                                                          : [...current, field];
                                                      setCurrentSection({...currentSection, fields: updated});
                                                  }}
                                              />
                          ...
                          <div className="mt-6 flex justify-end gap-2">
                              ...
                              <button onClick={handleSaveSection} className="px-4 py-2 bg-slate-900 text-white rounded font-bold text-sm">Guardar Sección</button>
                          </div>
                      </div>
                  </div>
              )}
              {isFieldModalOpen && (
                  <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6">
                          <div className="flex justify-between items-center mb-4">
                              <h3 className="text-lg font-bold text-slate-800">{currentField.id ? 'Editar Campo' : 'Nuevo Campo'}</h3>
                          ...
                          <div className="space-y-4">
                              ...
                              <div className="flex items-center space-x-2">
                                  ...
                                  />
                                  <label htmlFor="field-required" className="text-sm font-bold text-slate-700">Obligatorio</label>
                              </div>
                          </div>
                          <div className="mt-6 flex justify-end gap-2">
                              <button onClick={() => setIsFieldModalOpen(false)} className="px-4 py-2 text-slate-600 font-bold text-sm">Cancelar</button>
                              <button onClick={handleSaveField} className="px-4 py-2 bg-slate-900 text-white rounded font-bold text-sm">Guardar Campo</button>
                          </div>
                      </div>
                  </div>
              )}
