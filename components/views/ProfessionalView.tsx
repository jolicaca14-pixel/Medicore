...
export const ProfessionalView: React.FC<ProfessionalViewProps> = ({ user, activeTab = 'dashboard' }) => {
  ...
  const handleDownloadContract = () => {
      ...
      const contractText = `
        ...
        CONTRATANTE: MediCore IPS SAS
        CONTRATISTA: ${user.name}
        IDENTIFICACIÓN: ${user.documentNumber}

        Por medio del presente se certifica la vinculación vigente...
                                      ...
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              ...
                                              <span className="text-xs text-slate-600">
                                                  {newPayment.files.includes('ss.pdf') ? 'ss.pdf ✅' : 'Planilla Seguridad Social'}
                                              </span>
                                          </div>
                                          <button
                                            onClick={() => setNewPayment(prev => ({ ...prev, files: [...prev.files, 'ss.pdf'] }))}
                                            className={`text-xs px-2 py-1 rounded border transition-colors ${newPayment.files.includes('ss.pdf') ? 'bg-green-50 text-green-700 border-green-200' : 'bg-white hover:bg-slate-100'}`}
                                          >
                                              {newPayment.files.includes('ss.pdf') ? 'Cargado' : 'Seleccionar...'}
                                          </button>
                                      </div>
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              <FileText size={16} className="text-slate-400 mr-2"/>
                                              <span className="text-xs text-slate-600">
                                                  {newPayment.files.includes('informe.pdf') ? 'informe.pdf ✅' : 'Informe de Actividades'}
                                              </span>
                                          </div>
                                          <button
                                            onClick={() => setNewPayment(prev => ({ ...prev, files: [...prev.files, 'informe.pdf'] }))}
                                            className={`text-xs px-2 py-1 rounded border transition-colors ${newPayment.files.includes('informe.pdf') ? 'bg-green-50 text-green-700 border-green-200' : 'bg-white hover:bg-slate-100'}`}
                                          >
                                              {newPayment.files.includes('informe.pdf') ? 'Cargado' : 'Seleccionar...'}
                                          </button>
                                      </div>
                                  </div>
                              </div>
