                                  ...
                                  <div className="space-y-2">
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              ...
                                              <span className="text-xs text-slate-600">{newPayment.files.includes('ss_upload.pdf') ? 'ss_upload.pdf' : 'Planilla Seguridad Social'}</span>
                                          </div>
                                          <button onClick={() => setNewPayment(prev => ({ ...prev, files: [...prev.files, 'ss_upload.pdf'] }))} className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100">
                                              {newPayment.files.includes('ss_upload.pdf') ? 'Cambiar' : 'Seleccionar'}
                                          </button>
                                      </div>
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              <FileText size={16} className="text-slate-400 mr-2"/>
                                              <span className="text-xs text-slate-600">{newPayment.files.includes('act_report.pdf') ? 'act_report.pdf' : 'Informe de Actividades'}</span>
                                          </div>
                                          <button onClick={() => setNewPayment(prev => ({ ...prev, files: [...prev.files, 'act_report.pdf'] }))} className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100">
                                               {newPayment.files.includes('act_report.pdf') ? 'Cambiar' : 'Seleccionar'}
                                          </button>
                                      </div>
                                  </div>
                              </div>
