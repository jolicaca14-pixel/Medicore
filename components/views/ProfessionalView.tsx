                                  ...
                                  <div className="space-y-2">
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              {newPayment.files.includes('ss_ok') ? <CheckCircle size={16} className="text-green-500 mr-2"/> : <FileText size={16} className="text-slate-400 mr-2"/>}
                                          ...
                                          <label className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100 cursor-pointer">
                                              {newPayment.files.includes('ss_ok') ? 'Cambiado' : 'Seleccionar...'}
                                              <input type="file" className="hidden" onChange={() => setNewPayment(prev => ({...prev, files: [...prev.files, 'ss_ok']}))} />
                                          </label>
                                      </div>
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              {newPayment.files.includes('act_ok') ? <CheckCircle size={16} className="text-green-500 mr-2"/> : <FileText size={16} className="text-slate-400 mr-2"/>}
                                              <span className="text-xs text-slate-600">Informe de Actividades</span>
                                          </div>
                                          <label className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100 cursor-pointer">
                                              {newPayment.files.includes('act_ok') ? 'Cambiado' : 'Seleccionar...'}
                                              <input type="file" className="hidden" onChange={() => setNewPayment(prev => ({...prev, files: [...prev.files, 'act_ok']}))} />
                                          </label>
                                      </div>
                                  </div>
                              </div>
