...
export const ProfessionalView: React.FC<ProfessionalViewProps> = ({ user, activeTab = 'dashboard' }) => {
  ...
  const handleSaveDescargos = () => {
      ...
      setStatusMessage({ text: "Sus descargos han sido registrados correctamente.", type: 'success' });
  ...
  const handleOpenPaymentModal = () => {
      ...
      if (!activeContract) {
          setStatusMessage({ text: "Solo disponible para contratos OPS Activos.", type: 'error' });
          setTimeout(() => setStatusMessage(null), 3000);
          return;
      }
  ...
  const handleGeneratePaymentRequest = () => {
      ...
      setStatusMessage({ text: "Cuenta de cobro generada y notificada a Administración.", type: 'success' });
      setTimeout(() => setStatusMessage(null), 3000);
  ...
  const initiateAuth = (action: 'FINALIZE' | 'SIGN_NOTE') => {
    if (action === 'FINALIZE') {
        ...
        if (selectedPatient.gender === 'M' && (selectedTemplate?.recordType === RecordType.PYP_PREGNANCY || selectedTemplate?.recordType === RecordType.PYP_PUERPERIUM)) {
            setStatusMessage({ text: "Error: Plantillas de control prenatal no aplicables a hombres.", type: 'error' });
            setTimeout(() => setStatusMessage(null), 3000);
            return;
        }
        ...
        if (!currentRecord.antecedents || currentRecord.antecedents.trim() === '') {
            // Using browser confirm for critical safety check, though we could use a custom modal later
            const confirmEmpty = window.confirm("Atención: Los antecedentes clínicos están vacíos. ¿Desea continuar?");
            if (!confirmEmpty) {
                if (selectedTemplate?.sections?.length) {
                    setActiveFormTab(selectedTemplate.sections[0].id);
                }
        ...
        if ((!currentRecord.diagnoses || currentRecord.diagnoses.length === 0) && selectedTemplate?.recordType !== RecordType.PROCEDURE) {
            setStatusMessage({ text: "Es obligatorio seleccionar al menos un diagnóstico CIE-11.", type: 'error' });
            setTimeout(() => setStatusMessage(null), 3000);
        ...
        if (isFirstTimeRCV && selectedTemplate?.recordType === RecordType.PYP_CV_RISK) {
            if(!dynamicData['global_barthel']) {
                setStatusMessage({ text: "La Escala de Barthel es obligatoria para el programa de RCV.", type: 'error' });
                setTimeout(() => setStatusMessage(null), 3000);
                return;
            }
        }
    ...
    if (passwordInput === user.documentNumber) {
      if (authAction === 'FINALIZE') {
        ...
        setTimeout(() => {
          ...
          setViewMode('LIST');
          setSelectedPatient(null);
          setStatusMessage({ text: "Historia finalizada y RDA enviado exitosamente.", type: 'success' });
  ...
  const handleAddDiagnosis = (code: string, name: string) => {
      if (!validateCIE11Code(code)) {
          setStatusMessage({ text: "Código CIE-11 no válido para este paciente.", type: 'error' });
          setTimeout(() => setStatusMessage(null), 3000);
          return;
      }
  ...
  const handleImportResult = (result: ClinicalRecord) => {
      ...
      setStatusMessage({ text: `Datos de ${result.chiefComplaint} importados correctamente.`, type: 'success' });
      setTimeout(() => setStatusMessage(null), 3000);
  };
