                                      ...
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              <FileText size={16} className="text-slate-400 mr-2"/>
                                                  <span className="text-xs text-slate-600">{newPayment.files.some(f => f.includes('seg_social')) ? 'seg_social.pdf' : 'Planilla Seguridad Social'}</span>
                                          </div>
                                              <label className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100 cursor-pointer">
                                                  {newPayment.files.some(f => f.includes('seg_social')) ? 'Cambiar' : 'Seleccionar...'}
                                                  <input type="file" className="hidden" onChange={() => { setNewPayment(prev => ({...prev, files: [...prev.files, 'seg_social.pdf']})); showStatus("Archivo seleccionado: seg_social.pdf"); }} />
                                              </label>
                                      </div>
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              <FileText size={16} className="text-slate-400 mr-2"/>
                                                  <span className="text-xs text-slate-600">{newPayment.files.some(f => f.includes('actividades')) ? 'actividades.pdf' : 'Informe de Actividades'}</span>
                                          </div>
                                              <label className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100 cursor-pointer">
                                                  {newPayment.files.some(f => f.includes('actividades')) ? 'Cambiar' : 'Seleccionar...'}
                                                  <input type="file" className="hidden" onChange={() => { setNewPayment(prev => ({...prev, files: [...prev.files, 'actividades.pdf']})); showStatus("Archivo seleccionado: actividades.pdf"); }} />
                                              </label>
                                      </div>
                                  </div>
                              </div>
