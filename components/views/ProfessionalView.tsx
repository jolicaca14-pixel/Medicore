                                      ...
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              ...
                                              <span className="text-xs text-slate-600">
                                                  {newPayment.files.find(f => f.includes('Seguridad')) ? '✅ Seguridad_Social.pdf' : 'Planilla Seguridad Social'}
                                              </span>
                                          </div>
                                          <button onClick={() => setNewPayment({...newPayment, files: [...newPayment.files, 'Seguridad_Social.pdf']})} className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100">Seleccionar...</button>
                                      </div>
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              <FileText size={16} className="text-slate-400 mr-2"/>
                                              <span className="text-xs text-slate-600">
                                                  {newPayment.files.find(f => f.includes('Actividades')) ? '✅ Informe_Actividades.pdf' : 'Informe de Actividades'}
                                              </span>
                                          </div>
                                          <button onClick={() => setNewPayment({...newPayment, files: [...newPayment.files, 'Informe_Actividades.pdf']})} className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100">Seleccionar...</button>
                                      </div>
                                  </div>
                              </div>
                          </div>
