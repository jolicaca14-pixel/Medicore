...
export const ProfessionalView: React.FC<ProfessionalViewProps> = ({ user, activeTab = 'dashboard' }) => {
  ...
  const handleDownloadContract = () => {
      ...
      const content = `CONTRATO DE VINCULACION\nFuncionario: ${user.name}\nDocumento: ${user.documentNumber}\nTipo: ${activeContract?.type}\nFecha: ${activeContract?.startDate}`;
                                  ...
                                  <div className="space-y-2">
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              ...
                                              <span className="text-xs text-slate-600">{newPayment.files.includes('ss.pdf') ? 'ss.pdf' : 'Planilla Seguridad Social'}</span>
                                          </div>
                                          <button onClick={() => setNewPayment({...newPayment, files: [...newPayment.files, 'ss.pdf']})} className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100">{newPayment.files.includes('ss.pdf') ? 'Cambiado' : 'Seleccionar...'}</button>
                                      </div>
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              <FileText size={16} className="text-slate-400 mr-2"/>
                                              <span className="text-xs text-slate-600">{newPayment.files.includes('info.pdf') ? 'info.pdf' : 'Informe de Actividades'}</span>
                                          </div>
                                          <button onClick={() => setNewPayment({...newPayment, files: [...newPayment.files, 'info.pdf']})} className="text-xs bg-white border px-2 py-1 rounded hover:bg-slate-100">{newPayment.files.includes('info.pdf') ? 'Cambiado' : 'Seleccionar...'}</button>
                                      </div>
                                  </div>
                              </div>
                          </div>
