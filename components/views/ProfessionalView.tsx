...
export const ProfessionalView: React.FC<ProfessionalViewProps> = ({ user, activeTab = 'dashboard' }) => {
  ...
  const handleDownloadContract = () => {
      ...
      const content = `CONTRATO DE PRESTACIÓN DE SERVICIOS PROFESIONALES\n\nContratista: ${user.name}\nDocumento: ${user.documentNumber}\n\nEste es un documento generado automáticamente por MediCore Pro como soporte legal de su vinculación.`;
      const blob = new Blob([content], { type: 'text/plain' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `contrato_${user.username}.txt`;
      link.click();
                                  ...
                                  <div className="space-y-2">
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              <FileText size={16} className={`mr-2 ${newPayment.files.includes('Seguridad_Social.pdf') ? 'text-green-500' : 'text-slate-400'}`}/>
                                              <span className="text-xs text-slate-600">Planilla Seguridad Social</span>
                                          </div>
                                          <button onClick={() => setNewPayment(prev => ({ ...prev, files: [...prev.files, 'Seguridad_Social.pdf'] }))} className={`text-xs px-2 py-1 rounded transition-colors ${newPayment.files.includes('Seguridad_Social.pdf') ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-white border hover:bg-slate-100'}`}>
                                              {newPayment.files.includes('Seguridad_Social.pdf') ? 'Cargado' : 'Seleccionar...'}
                                          </button>
                                      </div>
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                              <FileText size={16} className={`mr-2 ${newPayment.files.includes('Informe_Actividades.pdf') ? 'text-green-500' : 'text-slate-400'}`}/>
                                              <span className="text-xs text-slate-600">Informe de Actividades</span>
                                          </div>
                                          <button onClick={() => setNewPayment(prev => ({ ...prev, files: [...prev.files, 'Informe_Actividades.pdf'] }))} className={`text-xs px-2 py-1 rounded transition-colors ${newPayment.files.includes('Informe_Actividades.pdf') ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-white border hover:bg-slate-100'}`}>
                                              {newPayment.files.includes('Informe_Actividades.pdf') ? 'Cargado' : 'Seleccionar...'}
                                          </button>
                                      </div>
                                  </div>
                              </div>
