...
export const ProfessionalView: React.FC<ProfessionalViewProps> = ({ user, activeTab = 'dashboard' }) => {
  ...
  const handleDownloadContract = () => {
      ...
      const content = `
          ...
          ESTADO: ${activeContract?.isActive ? 'ACTIVO' : 'INACTIVO'}

          Este documento es una copia generada digitalmente de su contrato vigente.
      `;
                              ...
                              <div className="border-t pt-4">
                                  ...
                                  <div className="space-y-2">
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          ...
                                          <button
                                            onClick={() => setNewPayment(prev => ({ ...prev, files: [...prev.files, 'seguridad_social_mock.pdf'] }))}
                                            className={`text-xs px-2 py-1 rounded font-bold transition-colors ${newPayment.files.includes('seguridad_social_mock.pdf') ? 'bg-green-100 text-green-700' : 'bg-white border hover:bg-slate-100'}`}
                                          >
                                            {newPayment.files.includes('seguridad_social_mock.pdf') ? '✓ Cargado' : 'Seleccionar...'}
                                          </button>
                                      </div>
                                      <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-dashed border-slate-300">
                                          <div className="flex items-center">
                                          ...
                                          <button
                                            onClick={() => setNewPayment(prev => ({ ...prev, files: [...prev.files, 'informe_actividades_mock.pdf'] }))}
                                            className={`text-xs px-2 py-1 rounded font-bold transition-colors ${newPayment.files.includes('informe_actividades_mock.pdf') ? 'bg-green-100 text-green-700' : 'bg-white border hover:bg-slate-100'}`}
                                          >
                                            {newPayment.files.includes('informe_actividades_mock.pdf') ? '✓ Cargado' : 'Seleccionar...'}
                                          </button>
                                      </div>
                                  </div>
                              </div>
